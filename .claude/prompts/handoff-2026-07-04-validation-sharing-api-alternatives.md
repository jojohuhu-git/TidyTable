...
**The mission is to level these users up, not to route around their limits.** Joanne's goal (stated 2026-07-04): an Excel novice should be able to run complex, analyst-grade data cleaning independently — effectively replacing a data analyst. Complex formulas are *welcome and wanted*, as long as every formula arrives fully executable: the exact formula text, the exact cell to type it in, the exact range to fill it down to, and a plain-English sentence on what it does. Don't dumb the work down to filter-and-count; make the advanced work followable, and let skills accumulate.

When judging any feature or output in this handoff, the test is: *could someone whose whole toolkit today is "filter and manually count" execute this correctly on the first try, and understand a little more Excel than they did before?*
...
a. **Excel steps should teach analyst-grade formulas, fully placed and fully explained.** Formulas are the point, not the fallback — the user should finish a session having *done* real data cleaning in Excel, not having avoided it. Update the system prompt (`src/logic/claude.js`) so `excel_steps` follows this contract:
   - **Every formula is fully executable with zero prior knowledge:** the exact formula text to type, the exact cell to put it in ("click cell H2, type the formula, press Enter"), the exact fill range ("then copy it down to H251: click H2, then double-click the small square at its bottom-right corner — the 'fill handle'"), and which sheet it lives on. Never say "add a formula like…" — always the literal, complete formula for *their* columns and row counts (both are provided in the request context).
   - **Every formula gets a one-sentence plain-English translation** immediately after it, e.g. *"=COUNTIFS(D2:D251, \">65\", F2:F251, \"\") means: count the rows where column D is over 65 AND column F is empty."* First use of any mechanic (formula bar, pressing Enter, fill handle, absolute references with $) gets taught inline; later uses can just reference it.
   - **Use the full data-cleaning toolbox where it genuinely fits** — COUNTIFS/SUMIFS/AVERAGEIFS, IF/IFS, TRIM/PROPER/SUBSTITUTE for text cleanup, VALUE/TEXT for type fixes, XLOOKUP or VLOOKUP for cross-sheet matching, COUNTIF duplicate flags, IFERROR, plus built-in tools like Remove Duplicates and Text to Columns when they're the honest analyst move. Prefer the simplest formula that does the job (this is a teaching constraint, not a capability ceiling), flag anything 365-only, and note Windows/Mac differences only where the clicks differ.
   - **Skill accumulation:** where natural, add a short "what you just learned" line at the end of the recipe ("you now know how to count rows that meet several conditions at once — COUNTIFS works the same for any columns"). Filter-and-count and the status-bar Count trick are still fine as a quick *cross-check* step, but they are the sanity check, not the method.
   - **UI support (small build task):** in the "Check it in Excel" tab, consider an expandable "how do I type a formula?" primer (formula bar, Enter, fill handle, with a screenshot or small illustration) shown once above the steps, so individual steps stay tight.
...
b. **Jargon audit of everything the user reads.** Two surfaces:
   ...
   - **Important scoping (per Joanne's 2026-07-04 direction):** Excel formula names (COUNTIFS, XLOOKUP, TRIM…) are NOT jargon — they are the curriculum, and they must appear, each explained in plain English on first use per recipe. The ban-list targets *programming/AI* vocabulary the user never needs: "string", "boolean", "parse", "null", "array", "token", "schema", "transform", "dataframe", "JSON". "Formula", "cell", "column", "sheet" are of course fine.
